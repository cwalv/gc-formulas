#!/usr/bin/env bash
# eval-gc.sh — plan-evals C.2 substrate runner under the gc shim.
#
# Runs a plan-evals case through the validation-pack container, where workers
# execute INSIDE the container under a real gc supervisor + bd substrate (not
...
#
# Usage:
#   bash scripts/eval-gc.sh <case-id> [--output-dir DIR] [--run-id ID] \
...
CASE_ID=""
# Default under the repo root so snap-docker bind mounts resolve. /tmp is not
# visible to snap-confined docker; bind-mounting a /tmp path silently materialises
# a root-owned 755 dir inside the container that uid 1000 cannot write to.
OUTPUT_DIR=""
RUN_ID=""
...
if [[ ! -f "$FANOUT_CONFIG" ]]; then
    echo "${CASE_ID}/fanout.json required for eval-gc.sh (must declare 'dir')" >&2
    exit 1
fi
...
if [[ -z "$RUN_ID" ]]; then
    RUN_ID="gc-${PATTERN}-${CASE_ID}-$(date -u +%Y%m%d-%H%M%S)"
fi

if [[ -z "$OUTPUT_DIR" ]]; then
    OUTPUT_DIR="${REPO_ROOT}/eval-runs"
fi
...
COMPOSE_FILES=(-f "${PACK_DIR}/docker-compose.yml" -f "${PACK_DIR}/docker-compose.eval.yml")
EXTRA_MOUNTS=()
...
PYEOF
    ...
    echo "[eval-gc] worker-model override: ${WORKER_MODEL_OVERRIDE}" >&2
fi

PROJECT_NAME="vp-eval-$(echo "${RUN_ID}" | tr -c '[:alnum:]_-' '_' | cut -c1-50)"

echo "[eval-gc] case=${CASE_ID} pattern=${PATTERN} run-id=${RUN_ID}" >&2
echo "[eval-gc] worktree=${WORKTREE}" >&2
echo "[eval-gc] fanout dir=${FANOUT_DIR} exclude='${FANOUT_EXC}'" >&2
...
echo "[eval-gc] container exited rc=${CONTAINER_EXIT} wall=${WALL_SECS}s" >&2
...
if [[ -f "$AGGREGATOR" && -d "$CLAUDE_PROJECTS_DIR" ]]; then
    ...
    if [[ -n "$AGG_JSON" ]]; then
        ...
        echo "[eval-gc] token aggregation: tokens_in=${TOKENS_IN} tokens_out=${TOKENS_OUT} cache_creation=${CACHE_CREATION} cache_read=${CACHE_READ}" >&2
    fi
else
    echo "[eval-gc] token aggregation skipped: claude-projects dir not found at ${CLAUDE_PROJECTS_DIR}" >&2
fi
...
echo "[eval-gc] results: ${RESULTS_FILE}" >&2
exit "${CONTAINER_EXIT}"
